...
if (USERNAME === undefined || PASSWORD === undefined) {
  ...
  console.error("如果你在 GitHub Actions 运行，请检查 workflow 的 env 配置以及 Secrets 是否正确绑定。");
  process.exit(1); // 异常退出
}
...
async function saveCookieJar(jar: CookieJar): Promise<void> {
  ...
  await fs.writeFile(COOKIE_FILE, JSON.stringify(serialized, null, 2), 'utf-8');
}
...
// === 主流程 ===
export async function login_for_data(type: "humanity" | "science"): Promise<string | undefined> {
  ...
  try {
    // 2. 尝试直接访问最终目标页
    const testRes = await client.get(TARGET_URL[type]);
    ...
    console.log(`⚠️ 网络请求异常或 Session 失效 (${e.message})，准备重新登录...`);
  }
  ...
  try {
    ...
    for (let attempt = 1; attempt <= MAX_LOGIN_ATTEMPTS; attempt++) {
      ...
      if (resHome.data.includes('验证码')) {
        ...
        while (true) {
          ...
          // 请求时带上时间戳避免缓存
          const captchaRes = await client.get(`https://sep.ucas.ac.cn/changePic?code=${timestamp}`, {
          ...
          if (/^[A-Za-z0-9]{4}$/.test(recognizedText)) {
            ...
            console.log(`⚠️ 识别结果异常 (${recognizedText})，图像过于复杂，正在刷新验证码重试...`);
      ...
      const loginRes = await client.post('https://sep.ucas.ac.cn/slogin', loginData.toString(), {
        headers: {
      ...
      if (finalUrl === 'https://sep.ucas.ac.cn/' || loginRes.data.includes('错误') || loginRes.data.includes('无效')) {
        console.log(`❌ 第 ${attempt} 次登录失败。可能原因：密码错误、验证码识别错误或被风控。`);

        if (attempt === MAX_LOGIN_ATTEMPTS) {
          throw new Error(`已达到最大尝试次数 (${MAX_LOGIN_ATTEMPTS} 次)，程序退出。`);
        }
    ...
    const menuRegex = new RegExp(`href="(\\/portal\\/site\\/[^"]+)"[^>]*>${TARGET_NAME[type]}`);
    ...
    const finalRes = await client.get(ssoUrl);
    ...
    if(finalUrl!==TARGET_URL[type]){
      console.error("最终链接路径错误(Real !== Expected):",finalUrl,"!==",TARGET_URL[type]);
    ...
    if (!is_invalid(finalRes.data)) {
      ...
      console.error("❌ 流程执行完毕，但最终页面内容仍然无效。可能是网络波动或学校系统结构更新。");
    }
